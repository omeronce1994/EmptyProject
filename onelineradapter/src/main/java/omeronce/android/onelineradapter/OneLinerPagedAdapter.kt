...
class OneLinerPagedAdapter<T> protected constructor(diffUtil: DiffUtil.ItemCallback<T>, builder:Builder<T>): PagedListAdapter<T, GenericViewHolder>(diffUtil) {
    ...
    class Creator<T>(private val diffUtil: DiffUtil.ItemCallback<T>, private val builder: Builder<T>) {
        fun build() =  OneLinerPagedAdapter(diffUtil, builder)
    }
}